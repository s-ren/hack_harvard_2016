...
var ignore = ['login-page','home-page','signup-page','signupwait-page','resetpwdwait-page','forgetpwd-page']
//var resetEmail = "";
...
$(document).on("pageshow", "#login-page", function(){
	...
	$("#login-user").keyup(function(){
		...
		console.log(email);
		//$("#login-user").val(email+'@brown.edu');
	})
...
$(document).on("pageshow", "#orders-page", function(){
	...
	$.post("/getdeals", postParameters, function(responseJSON){
		...
		if (orders.length != 0 || offers.length != 0) {
			$("#curr-label").html('Current' + '    <span style="color: #ff44b6; font-family: Arial; font-size: 0.8em">' + (orders.length + offers.length) + '</span>');
		}
...
$(document).on("pageshow", "#profile-page", function(){
	...
	$(document).on('blur', '#profile-contact-in', function(){
		...
		if(checkValidPhone(num)) {
			sessionStorage.contact = newContact;
		} else {
			$("#profile-error").text('Please input a valid US phone number.');
		}
...
$( document ).on( "pageshow", "#page_buy_3", function(event) {
  ...
  //$.post("/getmenu",{eatery:localStorage.eatery},function(response){
  	...
  	var menu = ["Pho","Milk","Tea","Coffee","Sushi","Brownie"];
