	...
	$("#login-submit").click(function(){
		...
		$.post("/userlogin",postParameters,function(responseJSON){
			...
			if (JSON.parse(responseJSON).error != undefined) {
				...
				if (JSON.parse(responseJSON).comment) {
					//$(".alert-p").html();
					$(".alert-p").html('Orders &#10071;');
				}
				// $(".alert-p").html();
		...
		if (history.length != 0) {
			...
			for (var p = 0; p < history.length; p++) {
				...
				var li = document.createElement("li");
	         	...
	         	li.innerHTML = '<h3>'+ buyOrSell  + ' credit to</h3>' + 
		         	...
		         	$("#" + trimedTime + "commentbtn").click(function(){
		         		...
		         		li.innerHTML = '<form id="' + commentID + 'form">' + 
        					...
        					'</div>' + 
              				...
              				'style="height: 52px" ' + 
              				'placeholder="please comment on the seller/buyer.">' + 
              				'</textarea>' + 
 ...
 //  var intervalFlag = 1;
  ...
  $("#final_confirm").tap(function(){
   ...
   //      	var currTime = currDate.getDate() + '/' + 
	  ...
	  //       li.innerHTML = '<a href="#" class="ui-btn" >' + 
	  //        			'<h3>Buy Pending:</h3><p style="font-size: 15px; text-align: left"></p>' + 
	  //        			'Eatery: ' + request.eatery + 
	  //        			'Duration: ' + request.duration + 
	  //        			'Time: ' + currTime;
	  //        			'<a href="#delete-popup" data-rel="popup" data-icon="delete" aria-haspopup="true" aria-owns="delete-popup" aria-expanded="false" class="ui-btn ui-btn-icon-notext ui-icon-delete" title=“”>::after</a>';
	  //       $("#orders-ul").append(li);
	     ...
	     //    var li = document.createElement("li");
      //    	li.setAttribute("class", "ui-li-static ui-body-inherit ui-first-child ui-last-child");
      //    	li.innerHTML = '<h3>Bought 1 credit to</h3>' + 
	     //     	'<p style="font-size: 15px">' + 
	     //     	data.sellerName + 
	     //     	'<br>Contact: ' + data.sellerContact + 
	     //     	'<br>Price: ' + data.price + 
	     //     	'<br>' + 
	     //     	'Dining Hall: ' + data.eatery + 
	     //     	'<br>' + 
	     //     	'Time: ' + data.time;
	     //     	$("#deals-history").append(li);
         ...
         // 	var profileComment = '<li class="ui-li-static ui-body-inherit ui-first-child">::before' + 
	...
	$("#final_confirm_sell").click(function(){
	   ...
	   //      	li.setAttribute("class", "ui-li-has-alt ui-first-child ui-last-child");
		  // //       li.addEventListener("tap", function(e){
				...
				// console.log('currtime: ' + currTime);
		  //       li.innerHTML = '<a href="#" class="ui-btn" >' + 
		  //        			'<h3>Sell Pending:</h3><p style="font-size: 15px; text-align: left"></p>' + 
		  //        			'Eatery: ' + offer.eatery + 
		  //        			'Duration: ' + offer.duration + 
		  //        			'Time: ' + currTime;
		  //        			'<a href="#delete-popup" data-rel="popup" data-icon="delete" aria-haspopup="true" aria-owns="delete-popup" aria-expanded="false" class="ui-btn ui-btn-icon-notext ui-icon-delete" title=“”>::after</a>';
		  //       $("#orders-ul").append(li);
	         	...
	         	//put in orders history.
	         	// console.log('buyer name: ' + data.buyerName);
	         	// console.log('price: ' + data.price);
	         	// var li = document.createElement("li");
	         	// li.setAttribute("class", "ui-li-static ui-body-inherit ui-first-child ui-last-child");
	         	// li.innerHTML = '<h3>Sold 1 credit to</h3>' + 
	         	// '<p style="font-size: 15px">' + 
	         	// data.buyerName + 
	         	// '<br>' + 
